...
clean_df2 = df[df["valeurfonc_sum_cod111"].notna() & df["sbati_sum_cod111"].notna()]
...
grouped_sum = idf_clean_df2.groupby(["year", "departement"]).agg(
    valeurfonc_totale=("valeurfonc_sum_cod111", "sum"),
    surface_totale=("sbati_sum_cod111", "sum")
).reset_index()
...
plt.title("Évolution du prix moyen au m² (valeur foncière / surface) des maisons en Île-de-France")
plt.xlabel("Année")
